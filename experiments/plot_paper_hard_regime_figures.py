...
from paper_plot_style import clean_axis, configure_paper_plots, save_figure
    ...
    if metric == "test_mse":
        ...
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                ...
                ax.text(j + 0.5, i + 0.5, txt, ha="center", va="center", fontsize=6.6, color=color)
    else:
        ...
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                ...
                ax.text(j + 0.5, i + 0.5, txt, ha="center", va="center", fontsize=6.8, color=color)

    return im


def save_both(fig, out_base):
    save_figure(fig, Path(out_base))


def figure1_formula_phase(df, dim, out_dir):
    fig, axes = plt.subplots(1, 3, figsize=(6.7, 2.25), constrained_layout=True)
    ...
    cbar = fig.colorbar(im, ax=axes, shrink=0.82, pad=0.012)
...
def figure2_prediction_vs_formula_raw(df, dim, out_dir):
    fig, axes = plt.subplots(1, 2, figsize=(5.2, 2.3), constrained_layout=True)
    ...
    cbar0 = fig.colorbar(im0, ax=axes[0], shrink=0.82, pad=0.018)
    cbar0.set_label("$\\log_{10}$(test MSE)")
    cbar0.outline.set_visible(False)

    cbar1 = fig.colorbar(im1, ax=axes[1], shrink=0.82, pad=0.018)
...
def figure3_variable_vs_interaction_raw(df, dim, out_dir):
    ...
    fig_width = 6.7 if len(metrics) == 3 else 5.2
    fig, axes = plt.subplots(1, len(metrics), figsize=(fig_width, 2.25), constrained_layout=True)
...
def figure4_controls(df, dim, out_dir):
    ...
    cbar = fig.colorbar(im1, ax=axes, shrink=0.82, pad=0.012)
    cbar.set_label("Interaction F1")
    cbar.outline.set_visible(False)
